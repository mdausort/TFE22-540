            ...
            for l in range(len(list_atlas)):
                for k in range(worksheet.shape[0]):
                    ...
                    if worksheet[k,0] in list_atlas[l]:
                        print(str(worksheet[k,0]))
                        percentage_change_all[l,i] = worksheet[k,3]
...
def get_kmeans(all_metric, nb_clust, axes_names, affinity, display, save):
    ...
    if (save == True):
        fig.savefig("/CECI/proj/pilab/PermeableAccess/alcoolique_TnB32xGDr7h/Analyse /Plots/Clusters_3D.svg")
